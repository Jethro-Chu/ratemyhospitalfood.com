...
export default class GameEngine {
    ...
    loadLevel(level) {
        ...
        // Maze systems (Level 2 section 3)
        this.switches = (level.switches || []).map((s) => ({ ...s, on: false }));
        this.drips = (level.drips || []).map((d) => ({ ...d, t: (d.phase || 0) / 1000 }));
        this.gatePetals = 0;
        this.gateNeeded = (level.petals || []).filter((p) => p.gate).length;
    ...
    _step(dt) {
        ...
        if (this.boss) {
            if (!this._bossSeen && this.player.x > this.boss.arena.minX - 170) {
                this._bossSeen = true; this._emitHud();
            }
            this._updateBoss(dt);
        }
    ...
    _solids() {
        return this.platforms.filter((pl) => !pl.broken && (
            pl.type === 'solid' || pl.type === 'breakable' || pl.type === 'ramp' ||
            pl.type === 'parmesan' || pl.type === 'crust' || pl.type === 'door' || pl.type === 'gate'
        ));
    }
    ...
    _updateProjectiles(dt) {
        for (const pr of this.projectiles) {
            ...
            pr.vy += 320 * dt;
            pr.life -= dt;
            // hit breakable blocks + burnt-crust blocks
            for (const pl of this.platforms) {
                if (!pl.broken && (pl.type === 'breakable' || pl.type === 'crust') && aabb(pr, pl)) {
                    this._breakBlock(pl); pr.life = 0;
                }
            }
            // hit sauce switches (open linked ravioli doors)
            for (const s of this.switches) {
                if (!s.on && aabb(pr, s)) { s.on = true; this._openDoors(s.id); pr.life = 0; }
            }
            // hit enemies
    ...
    _updateDrips(dt) {
        for (const d of this.drips) {
            d.t -= dt;
            if (d.t <= 0) {
                d.t = (d.period || 1800) / 1000;
                this.enemyShots.push({ x: d.x, y: d.top, w: 4, h: 6, vx: 0, vy: 50, life: 4, kind: 'drip' });
            }
        }
    }
    ...
    _openDoors(id) {
        let opened = false;
        for (const pl of this.platforms) {
            if (pl.type === 'door' && pl.id === id && !pl.broken) {
                pl.broken = true; opened = true;
                this._poof(pl.x + pl.w / 2, pl.y + pl.h / 2, '#ff9a6a', 12);
            }
        }
        if (opened) { this.audio?.coin(); this._toast('Click!', 'A ravioli door swings open!'); }
    }

    _openGate() {
        let opened = false;
        for (const pl of this.platforms) {
            if (pl.type === 'gate' && !pl.broken) {
                pl.broken = true; opened = true;
                this._poof(pl.x + pl.w / 2, pl.y + pl.h / 2, '#ff8fae', 20);
            }
        }
        if (opened) {
            this.shake = 0.4;
            this.audio?.petal();
            this._toast('Princess Peach Tart', 'The great ravioli gate rumbles open. Follow the warm sauce, Matt!');
        }
    }
    ...
    _collectibles() {
        ...
        for (const pt of this.petals) {
            if (!pt.got && aabb(p, pt)) {
                pt.got = true; this.score += SCORE.petal; this.crumbs += 1; this.audio?.petal();
                if (pt.gate) {
                    this.gatePetals += 1;
                    if (this.gateNeeded > 0 && this.gatePetals >= this.gateNeeded) this._openGate();
                }
                this._emitHud();
    ...
    _drawPlatforms(ctx) {
        for (const pl of this.platforms) {
            ...
            else if (pl.type === 'crust') this._drawCrust(ctx, pl);
            else if (pl.type === 'door') this._drawDoor(ctx, pl);
            else if (pl.type === 'gate') this._drawGate(ctx, pl);
        }
        for (const m of this.mushrooms) {
            const sq = m.squish;
    ...
    _drawCrust(ctx, pl) {
        // Burnt pasta crust — sauce it to break through
        ctx.fillStyle = '#5a3418';
        ctx.fillRect(pl.x, pl.y, pl.w, pl.h);
        ctx.fillStyle = '#3a2010';
        for (let y = pl.y + 2; y < pl.y + pl.h; y += 5) {
            for (let x = pl.x + 2; x < pl.x + pl.w; x += 5) ctx.fillRect(x + (y % 2 ? 2 : 0), y, 2, 2);
        }
        ctx.fillStyle = '#7a4a24';
        ctx.fillRect(pl.x, pl.y, pl.w, 2);
        ctx.fillStyle = '#caa15a';
        ctx.fillRect(pl.x + pl.w / 2 - 1, pl.y + Math.floor(pl.h / 2), 2, 2);
    }

    _drawDoor(ctx, pl) {
        // Ravioli door (opens when its sauce switch is hit)
        ctx.fillStyle = '#f2c46a';
        ctx.fillRect(pl.x, pl.y, pl.w, pl.h);
        ctx.fillStyle = '#d89a3a';
        for (let y = pl.y; y < pl.y + pl.h; y += 4) ctx.fillRect(pl.x, y, pl.w, 1);
        ctx.fillStyle = '#fff0c0';
        ctx.fillRect(pl.x + pl.w / 2 - 2, pl.y + pl.h / 2 - 2, 4, 4);
        ctx.fillStyle = '#e23b34';
        ctx.fillRect(pl.x + pl.w / 2 - 1, pl.y + 6, 2, 5);
    }

    _drawGate(ctx, pl) {
        // The great ravioli gate — opens after all gate-petals are found
        ctx.fillStyle = '#e8a13c';
        ctx.fillRect(pl.x, pl.y, pl.w, pl.h);
        ctx.fillStyle = '#c47e28';
        for (let y = pl.y; y < pl.y + pl.h; y += 6) ctx.fillRect(pl.x, y, pl.w, 2);
        ctx.fillStyle = '#fff0c0';
        ctx.fillRect(pl.x + 1, pl.y, 2, pl.h);
        // progress pips (peach-petal locks)
        for (let i = 0; i < this.gateNeeded; i++) {
            const py = pl.y + 16 + i * 16;
            ctx.fillStyle = i < this.gatePetals ? '#ff8fae' : '#7a4a2a';
            ctx.fillRect(pl.x + pl.w / 2 - 3, py, 6, 6);
        }
    }
    ...
    _drawSwitches(ctx) {
        for (const s of this.switches) {
            ...
            ctx.fillRect(s.x - 1, s.y + s.h, s.w + 2, 2); // mount
            ctx.fillStyle = s.on ? '#7ad08a' : '#e23b34';
            ctx.fillRect(s.x, s.y, s.w, s.h);
            ctx.fillStyle = s.on ? '#aee6b6' : '#ff7a6f';
            ctx.fillRect(s.x + 1, s.y + 1, s.w - 2, 3);
        }
    }
    ...
    _drawDrips(ctx) {
        for (const d of this.drips) {
            ...
            ctx.fillRect(d.x - 3, d.top - 4, 8, 4);
            ctx.fillStyle = '#e23b34';
            const bob = (Math.sin(this.elapsed * 5 + d.x) + 1) * 1.5;
            ctx.fillRect(d.x, d.top, 2, 2 + bob);
        }
    }

    _drawHints(ctx) {
        for (const h of (this.level.hints || [])) {
            const glow = 0.55 + Math.sin(this.elapsed * 4 + h.x) * 0.3;
            ...
            if (h.dir === 'up') {
                ...
                ctx.beginPath(); ctx.moveTo(x + 8, y - 4); ctx.lineTo(x + 8, y + 4); ctx.lineTo(x + 13, y); ctx.closePath(); ctx.fill();
            }
        }
    }

    _drawCrumbTrail(ctx) {
        const tw = 0.45 + Math.sin(this.elapsed * 4) * 0.3;
        ctx.fillStyle = `rgba(255,143,174,${tw})`;
        for (const c of (this.level.crumbTrail || [])) ctx.fillRect(c.x, c.y, 2, 2);
    }

    _drawCavern(ctx) {
        for (const c of (this.level.caverns || [])) {
            ctx.fillStyle = 'rgba(74,46,26,0.5)';
            ctx.fillRect(c.x, c.y, c.w, c.h);
            // cheese crystals
            ctx.fillStyle = 'rgba(255,224,138,0.45)';
            for (let i = 20; i < c.w; i += 90) ctx.fillRect(c.x + i, c.y + 12, 3, 3);
            // tiny tomato lanterns
            const g = 0.4 + Math.sin(this.elapsed * 3) * 0.15;
            ctx.fillStyle = `rgba(255,90,80,${g})`;
            for (let i = 60; i < c.w; i += 150) ctx.fillRect(c.x + i, c.y + c.h - 34, 4, 4);
        }
    }
    ...
    _drawEnemyShots(ctx) {
        for (const s of this.enemyShots) {
            if (s.kind === 'drip') {
                ctx.fillStyle = '#e23b34';
                ctx.fillRect(Math.round(s.x), Math.round(s.y), 3, 5);
                ctx.fillStyle = '#ff7a6f';
                ctx.fillRect(Math.round(s.x), Math.round(s.y), 1, 2);
            } else {
                drawSprite(ctx, SPRITES.vinegar, s.x, s.y);
            }
        }
    }
